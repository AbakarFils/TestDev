...
class C_match extends MY_Controller
{
    ...
    { 
        ...
        else if($verification_match[0]->total==1 && $this->input->post('phase')==1 && $this->input->post('id_matchs')=='') //on verifie si les deux épuipes ont juste ont eu 1 matchs (aller simple)
        {
        ...
        {       //on edit le match encours (id_match!=null)
                ...
                {   // on effectue une nouvelle insertion dans la base
                ...
                $this->match->statut = 'inactif';

                $result = $this->match->save(); // save ou update 
                ...
                */
                ...
                if($result['status'] == 'success')
                {
                    ...
                    // mis à  jour d'un match encours d'edition
                    if( $stat===1)
                    {
                        
                        $id_equipe_b = $this->input->post('id_equipe_b');
                        $phase=$this->input->post('phase');
                        //cherche ce match par id_eqA ,id_eqB et phase
                        $match=$this->match->ckeck($id_equipe_a,$id_equipe_b, $phase); 

                        if($match==null)
                        {
                            $match=$this->match->ckeck($id_equipe_b,$id_equipe_a, $phase);
                        }
                       
                        $this->match->id_matchs = $match->id_matchs;
                        $this->match->get_record();

                        $this->classement->Match_joue=$this->classement->Match_joue;
                        ...
                        {
                            $this->classement->victoire =$this->classement->victoire;
                            $this->classement->pts= $this->classement->pts;
                        }
                        else if($this->input->post('score_a')==$this->input->post('score_b')) //null
                        {
                            $this->classement->nulls= $this->classement->nulls;
                            $this->classement->pts= $this->classement->pts;
                        }
                        else //defaite
                        {
                            $this->classement->defaite =$this->classement->defaite ;
                        }
                        $ok=1;
                        $this->classement->save();
                    }
                    else //nouveau match (etat==0 ie id_match==null)
                    {
                        $ok=0;
                        $this->classement->Match_joue+=1;
                        $this->classement->but_marque+=$this->input->post('score_a');
